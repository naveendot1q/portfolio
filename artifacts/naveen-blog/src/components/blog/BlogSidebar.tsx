  ...
  return (
    <aside className="hidden lg:flex flex-col gap-4" style={{ width: 280, flexShrink: 0 }}>
      <div className="card p-4" style={{ background: '#161b22' }}>
        <h3 className="font-display font-bold text-sm mb-4 pb-3 border-b" style={{ color: '#c9d1d9', borderColor: '#21262d', letterSpacing: '0.5px' }}>
          Writing Stats
        </h3>
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={label} className="rounded-lg p-3 text-center" style={{ background: '#21262d' }}>
      ...
      <div className="card p-4" style={{ background: '#161b22' }}>
        ...
        <div className="flex flex-col gap-1">
          <button onClick={() => onFilterChange('all')} className="flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all text-left"
            style={{ background: filter === 'all' ? 'rgba(255,107,26,.12)' : 'transparent', color: filter === 'all' ? '#FF6B1A' : '#8b949e' }}>
            <span>🗂️ All posts</span>
            <span className="text-xs font-mono px-1.5 py-0.5 rounded" style={{ background: '#21262d', color: '#8b949e' }}>{posts.length}</span>
          </button>
          {CATEGORIES.filter(c => catCounts[c.id]).map(cat => (
            <button key={cat.id} onClick={() => onFilterChange(cat.id)} className="flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all text-left"
              style={{ background: filter === cat.id ? `${cat.color}18` : 'transparent', color: filter === cat.id ? cat.color : '#8b949e' }}>
              <span>{cat.emoji} {cat.label}</span>
              <span className="text-xs font-mono px-1.5 py-0.5 rounded" style={{ background: '#21262d', color: '#8b949e' }}>{catCounts[cat.id]}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="card p-4" style={{ background: '#161b22' }}>
          ...
          <div className="flex flex-col gap-3">
            {recent.map(post => {
              ...
              return (
                <a key={post.id} href={`/blog/${post.slug}`} className="group flex flex-col gap-1 pb-3 border-b last:border-0 last:pb-0 transition-opacity hover:opacity-75" style={{ borderColor: '#21262d' }}>
                  <span className="text-sm leading-snug font-medium group-hover:text-orange-400 transition-colors" style={{ color: '#c9d1d9', lineHeight: 1.4 }}>
                    {post.title}
                  </span>
                  <span className="text-xs font-mono" style={{ color: '#8b949e' }}>{cat.emoji} {cat.label} · {d}</span>
                </a>
              );
            })}
          </div>
      ...
      <div className="card p-4" style={{ background: '#161b22' }}>
        ...
        <a href="https://www.linkedin.com/in/naveenmeel" target="_blank" rel="noopener" className="btn btn-outline text-xs py-1.5 px-3 w-full justify-center">
          LinkedIn ↗
        </a>
      </div>
    </aside>
